...
extern "C"
{
                                     ...
                                     uint   numCells)
    {
        ...
        reorderDataAndFindCellStartD<<< numBlocks, numThreads, smemSize>>>(
            cellStart,					// output: cell start index
            cellEnd,					// output: cell end index
            (float4 *) sortedPos,		// output: sorted positions
            (float4 *) sortedVel,		// output: sorted velocities
            gridParticleHash,			// input: sorted grid hashes
	...
	// Plot a field on the same grid used to sort the particles
	void particleField(	float *field,
						float *sortedPos,
						uint  *gridParticleIndex,
						uint  *cellStart,
						uint  *cellEnd,
						...
						uint  numGridCells)
	{
		...
		computeGridSize(numGridCells, 64, numBlocks, numThreads);

#if COMPUTE_FIELD_DERIVATIVE

		// execute the kernel
		particleField2D<<< numBlocks, numThreads >>>((float4 *)field,
			(float4 *)sortedPos,
			gridParticleIndex,
			cellStart,
			cellEnd,
...
#else
		// execute the kernel
		particleFieldD<<< numBlocks, numThreads >>>(field,
			(float4 *)sortedPos,
			gridParticleIndex,
			cellStart,
			cellEnd,
	...
	void clampField (float *field, uint  numGridCells)
	{
		// thread per grid cell
		uint numThreads, numBlocks;
		...
		// execute the kernel
		clampFieldD<<< numBlocks, numThreads >>>(field,  numGridCells);
