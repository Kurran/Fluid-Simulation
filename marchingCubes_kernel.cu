...
__device__ float4 sampleVectorVolume(float4 *data, uint3 p, uint3 gridSize)
{
    p.x = min(p.x, gridSize.x - 1);
    p.y = min(p.y, gridSize.y - 1);
    p.z = min(p.z, gridSize.z - 1);
...
__device__ float sampleVectorVolumeMag(float4 *data, uint3 p, uint3 gridSize)
{
    p.x = min(p.x, gridSize.x - 1);
    p.y = min(p.y, gridSize.y - 1);
    p.z = min(p.z, gridSize.z - 1);
...
// one thread per voxel
__global__ void
classifyVectorVoxel(uint *voxelVerts, uint *voxelOccupied, float4 *volume,
              uint3 gridSize, uint3 gridSizeShift, uint3 gridSizeMask, uint numVoxels,
              float3 voxelSize, float isoValue)
{
    uint blockId = __mul24(blockIdx.y, gridDim.x) + blockIdx.x;
    ...
    float field[8];
    field[0] = sampleVectorVolumeMag(volume, gridPos, gridSize);
    field[1] = sampleVectorVolumeMag(volume, gridPos + make_uint3(1, 0, 0), gridSize);
    field[2] = sampleVectorVolumeMag(volume, gridPos + make_uint3(1, 1, 0), gridSize);
    field[3] = sampleVectorVolumeMag(volume, gridPos + make_uint3(0, 1, 0), gridSize);
    field[4] = sampleVectorVolumeMag(volume, gridPos + make_uint3(0, 0, 1), gridSize);
    field[5] = sampleVectorVolumeMag(volume, gridPos + make_uint3(1, 0, 1), gridSize);
    field[6] = sampleVectorVolumeMag(volume, gridPos + make_uint3(1, 1, 1), gridSize);
    field[7] = sampleVectorVolumeMag(volume, gridPos + make_uint3(0, 1, 1), gridSize);
...
{
    // calculate number of vertices need per voxel
	classifyVectorVoxel<<<grid, threads>>>(voxelVerts, voxelOccupied, volume,
                                     gridSize, gridSizeShift, gridSizeMask,
                                     numVoxels, voxelSize, isoValue);
    getLastCudaError("classifyVectorVoxel failed");
}
...
// generate triangles for each voxel using marching cubes
// interpolates normals scalar field and its derivatives
__global__ void
generateTriangles3(float4 *pos, float4 *norm, uint *compactedVoxelArray, uint *numVertsScanned,  float4 *volume,
                  uint3 gridSize, uint3 gridSizeShift, uint3 gridSizeMask,
                  float3 voxelSize, float isoValue, uint activeVoxels, uint maxVerts)
{
    uint blockId = __mul24(blockIdx.y, gridDim.x) + blockIdx.x;
    ...
    float4 field[8];
    field[0] = sampleVectorVolume(volume, gridPos, gridSize);
    field[1] = sampleVectorVolume(volume, gridPos + make_uint3(1, 0, 0), gridSize);
    field[2] = sampleVectorVolume(volume, gridPos + make_uint3(1, 1, 0), gridSize);
    field[3] = sampleVectorVolume(volume, gridPos + make_uint3(0, 1, 0), gridSize);
    field[4] = sampleVectorVolume(volume, gridPos + make_uint3(0, 0, 1), gridSize);
    field[5] = sampleVectorVolume(volume, gridPos + make_uint3(1, 0, 1), gridSize);
    field[6] = sampleVectorVolume(volume, gridPos + make_uint3(1, 1, 1), gridSize);
    field[7] = sampleVectorVolume(volume, gridPos + make_uint3(0, 1, 1), gridSize);
...
launch_generateTriangles3(dim3 grid, dim3 threads,
                         ...
                         float3 voxelSize, float isoValue, uint activeVoxels, uint maxVerts)
{
    generateTriangles3<<<grid, NTHREADS>>>(pos, norm,
                                          compactedVoxelArray,
                                          numVertsScanned, volume,
                                          gridSize, gridSizeShift, gridSizeMask,
                                          voxelSize, isoValue, activeVoxels,
